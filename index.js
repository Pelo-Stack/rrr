...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('BOT X-PLAY', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('X-PLAY', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai minna~, Im BOT X-PLAY. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
